...
public class DebugSeven3
{
   ...
   { 
      ...
      quote = kb.nextLine();                         // FIXED: nextLine() → kb.nextLine()
      System.out.print("Enter a character to search for >> ");
      charString = kb.nextLine();                    // FIXED: nextInt() → nextLine()
      searchChar = charString.charAt(0);
      System.out.println("index of('" + searchChar + "') is: " +
         quote.indexOf(searchChar));                 // FIXED: missing ')'
      System.out.println("indexOf('a') is: " + quote.indexOf('a'));  // FIXED: indexOf'a'
      ...
      System.out.println("replace('e', '*') is: " +
         quote.replace('e', '*'));                   // FIXED: repplace → replace
   }
}